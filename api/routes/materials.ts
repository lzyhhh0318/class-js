...
import { db, type Material, type MaterialVersion } from '../db.js'
...
router.get('/preview', async (req: Request, res: Response) => {
  try {
    ...
    if (!response.ok) {
      res.status(response.status).json({ success: false, error: '获取文件失败' })
      return
    }
...
function detectMaterialType(fileName: string): Material['type'] {
  const ext = path.extname(fileName).toLowerCase()
  for (const [type, exts] of Object.entries(ALLOWED_EXTENSIONS)) {
    if (exts.includes(ext)) return type as Material['type']
  }
  return 'pdf'
}
...
router.get('/materials', (req: Request, res: Response) => {
  ...
  if (courseId) materials = materials.filter(m => m.courseId === courseId)
  if (chapterId) materials = materials.filter(m => m.chapterId === chapterId)
  ...
  if (userId) {
    ...
    if (user && user.role === 'student') {
      materials = materials.filter(m => {
        ...
        if (m.visibility === 'course') return user.courseIds.includes(m.courseId)
        return false
      })
    }
  }
...
router.get('/materials/:id', (req: Request, res: Response) => {
  ...
  db.accessLogs.push({
    ...
    userName: req.query.userName as string || '匿名',
...
router.post('/materials', upload.single('file'), (req: Request, res: Response) => {
  ...
  if (!req.file && ossUrl) {
    ...
    const detectedType = detectMaterialType(fileName || 'file.pdf')
  ...
  const existingByMd5 = Array.from(db.materials.values()).find(m =>
    m.versions.some(v => v.md5 === md5)
  )
  if (existingByMd5) {
...
router.put('/materials/:id', upload.single('file'), (req: Request, res: Response) => {
  const material = db.materials.get(req.params.id)
  if (!material) {
    res.status(404).json({ success: false, error: '资料不存在' })
    return
  ...
  if (visibility) material.visibility = visibility as Material['visibility']
...
router.get('/materials/:id/versions', (req: Request, res: Response) => {
  const material = db.materials.get(req.params.id)
  if (!material) {
    res.status(404).json({ success: false, error: '资料不存在' })
    return
...
router.post('/materials/:id/revert/:version', (req: Request, res: Response) => {
  const material = db.materials.get(req.params.id)
  if (!material) {
    res.status(404).json({ success: false, error: '资料不存在' })
    return
...
router.get('/materials/:id/download', (req: Request, res: Response) => {
  const material = db.materials.get(req.params.id)
  if (!material) {
    res.status(404).json({ success: false, error: '资料不存在' })
    return
  ...
  const version = parseInt(req.query.version as string) || material.currentVersion
