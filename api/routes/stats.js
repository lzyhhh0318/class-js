...
router.get('/stats/overview', (_req, res) => {
    ...
    db.accessLogs.forEach(l => {
        ...
        if (!dailyStats[day])
            dailyStats[day] = { views: 0, downloads: 0, uploads: 0, submissions: 0 };
        if (l.action === 'view')
            dailyStats[day].views++;
        if (l.action === 'download')
            dailyStats[day].downloads++;
        if (l.action === 'upload')
            dailyStats[day].uploads++;
        if (l.action === 'submit')
            dailyStats[day].submissions++;
    });
    res.json({
        success: true,
        data: {
...
router.get('/stats/material/:materialId', (req, res) => {
...
router.get('/stats/assignment/:assignmentId', (req, res) => {
