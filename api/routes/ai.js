...
dotenv.config();
const router = Router();
function getMockParsedContent(title) {
    return {
        materialId: '',
...
async function parseWithLLM(title, rawText) {
...
router.post('/materials/:id/parse', async (req, res) => {
...
router.get('/materials/:id/knowledge', (req, res) => {
    const material = db.materials.get(req.params.id);
    if (!material) {
        res.status(404).json({ success: false, error: '资料不存在' });
        return;
...
router.get('/courses/:courseId/knowledge', (req, res) => {
