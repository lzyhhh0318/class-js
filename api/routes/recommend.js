...
function pickGenericSuggestion(timeMinutes, scene, recentNames) {
...
function splitTaskIntoChunks(taskTitle, taskDesc, estimatedTime, timeMinutes) {
...
function getPhaseNames(title, desc, total) {
...
router.post('/', async (req, res) => {
    ...
    const getMockData = (timeMinutes, scene, tasks, lastIds) => {
        let taskName = "背英语单词";
        let reason = "时间短 + 高收益，随拿随放";
        let steps = ["1. 打开单词App", "2. 复习10个词", "3. 做一个小测试"];
        let sourceTaskId = undefined;
        let subTaskIndex = undefined;
        let newSubtasksToCache = undefined;
        if (tasks && tasks.length > 0) {
            const fittingTasks = tasks.filter((t) => t.estimatedTime <= timeMinutes);
            const recentSet = new Set(lastIds || []);
            if (fittingTasks.length > 0) {
                let bestTask = fittingTasks[0];
                let bestScore = -999;
                fittingTasks.forEach((t) => {
                    let score = 0;
                    score += t.progress;
                ...
                if (bestTask.subtasks && bestTask.subtasks.length > 0) {
                    const nextIdx = bestTask.subtasks.findIndex((st) => !st.completed);
                    if (nextIdx !== -1) {
